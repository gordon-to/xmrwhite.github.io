...
Creating the app itself is straightforward, and GitHub’s [documentation](https://docs.github.com/en/apps/creating-github-apps/creating-a-github-app) walks through it well.
...
You can use a single arc-system operator and multiple scale sets to deploy runners for multiple repos or orgs from a single cluster. 
This can also be used to create different flavours of runners if you want to create some runners that only have a small amount of CPU/RAM for linting and pr checks while reserving resources for things like large builds or e2e testing
